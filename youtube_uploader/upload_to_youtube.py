...
# 상위 폴더의 common_utils 모듈을 import하기 위한 경로 추가 - 최우선 설정
current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(current_dir)
utils_dir = os.path.join(parent_dir, 'utils')

# 모든 가능한 경로를 sys.path에 추가
...
print(f"DEBUG: Utils directory: {utils_dir}")
print(f"DEBUG: Python path: {sys.path[:5]}")

# 강력한 fallback import for different environments
try:
    print("DEBUG: Trying to import from utils.common_utils...")
    from utils.common_utils import get_gsheet
    print("DEBUG: Successfully imported from utils.common_utils")
except ImportError as e1:
    print(f"DEBUG: Failed to import from utils.common_utils: {e1}")
    try:
        print("DEBUG: Trying to import from common_utils...")
        from common_utils import get_gsheet
        print("DEBUG: Successfully imported from common_utils")
    except ImportError as e2:
        print(f"DEBUG: Failed to import from common_utils: {e2}")
        # 마지막 시도: 직접 파일 경로로 import
        try:
            import importlib.util
            spec = importlib.util.spec_from_file_location("common_utils", os.path.join(utils_dir, "common_utils.py"))
            if spec is None or spec.loader is None:
                raise ImportError("Could not create module spec")
            common_utils = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(common_utils)
            get_gsheet = common_utils.get_gsheet
            print("DEBUG: Successfully imported using importlib")
        except Exception as e3:
            print(f"DEBUG: All import methods failed: {e3}")
            raise ImportError("Could not import get_gsheet from any location")
